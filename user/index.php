...
$query1=mysqli_query($con,"SELECT * FROM users WHERE name='$username' AND password='$password'");
if($r = mysqli_fetch_assoc($query1))
{
	$_SESSION['dname']=$username;	
	
echo "<script>alert('Login Succesfully'); window.location = './view_doctor.php';</script>";
}
else
{
 ...
 <?php include('front_top.php');?>
  ...
  <a href="register_user.php" style="position:relative;left:40%;">Register</a>			
        </form>
        </div>
    </div>
</div>

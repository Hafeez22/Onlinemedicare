<div class="container">
 ...
 if(isset($_POST['submit']))
			...
			$query1=mysqli_query($con,"insert into add_doctor values('','$scat','$d_name','$qual',$exp,'$file','$addr','$mob')");
				if($r = $query1)
				{
    ...
    ?>
    <select class="form-control" name="s_category" id="country">
        <option value="">Select category</option>
        <?php
        if ($rowCount >  0) { 
            while($row = mysqli_fetch_assoc($query)){ 
                echo '<option value="'.$row['id'].'">'.$row['spec'].'</option>';
            }
        }
	   else {
